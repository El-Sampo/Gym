export default function SectionWrapper({children,header,title,id}) {
  return (
    <section id={id} className="min-h-screen flex flex-col gap-10">
        <div className="bg-white py-10 flex flex-col gap-4 justify-center items-center p-4">
            <p className="uppercase font-medium text-gray-500">
                {header}
            </p>
            <h2 className="font-semibold text-3xl sm:text-4xl md:text-5xl lg:text-6xl text-gray-900">
                {title[0]} <span className="uppercase text-red-600">{title[1]}</span> {title[2]}
            </h2>

        </div>
        <div className="max-w-[1000px] w-full flex flex-col mx-auto gap-10 p-4">
        {children}
        </div>

    </section>
  )
}

